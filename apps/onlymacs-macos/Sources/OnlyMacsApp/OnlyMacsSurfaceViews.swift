...
struct SwarmInviteManagementPanel: View {
    ...
    private var fullBody: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if let activePrivateSwarmName, !activePrivateSwarmName.isEmpty {
                Label(activePrivateSwarmName, systemImage: "lock.fill")
                    .font(.body.weight(.semibold))
            }
    ...
    private var privateSwarmTitle: String {
        let trimmed = activePrivateSwarmName?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return trimmed.isEmpty ? "Private Swarm" : trimmed
    }
}
